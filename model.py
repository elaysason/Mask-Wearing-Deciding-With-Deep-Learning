...
def evluate_hw1(test_loader):
    ...
    for (images, labels, names) in test_loader:
        ...
        all_outputs_test.extend(torch.exp(outputs).detach().numpy()[:, 1])
        all_labels_test.extend(labels.detach().numpy())
        _, predicted = torch.max(outputs.data, 1)
        loss = criterion(outputs, labels)
        cur_loss.append(loss.detach().numpy())
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels, names) in enumerate(dataloaders['train']):
            ...
            outputs = cnn(images)
            all_outputs_train.extend(torch.exp(outputs).detach().numpy()[:, 1])
            all_labels_train.extend(labels.detach().numpy())
            _, predicted = torch.max(outputs.data, 1)
            loss = criterion(outputs, labels)
            cur_loss.append(loss.detach().numpy())
            cur_f1.append(f1_score(labels.cpu(), predicted.cpu()))
        ...
        test_loss, test_f1, all_labels_test, all_outputs_test = evluate_hw1(dataloaders['test'])
        epoch_loss_test.append(test_loss)
        epoch_f1_test.append(test_f1)
        epoch_loss_train.append(sum(cur_loss) / len(cur_loss))
        epoch_f1_train.append(sum(cur_f1) / len(cur_f1))

    roc_graph(all_labels_train, all_outputs_train, 'Train')
    roc_graph(all_labels_test, all_outputs_test, 'Test')
    plt.show()
    plt.plot(np.arange(num_epochs), epoch_loss_train, label="Train")
    plt.plot(np.arange(num_epochs), epoch_loss_test, label="Test")
    ##plt.plot(np.arange(num_epochs), acc_test, label="test")
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.legend()
    ...
    plt.show()
    plt.plot(np.arange(num_epochs), epoch_f1_train, label="Train")
    plt.plot(np.arange(num_epochs), epoch_f1_test, label="Test")
    plt.xlabel("epoch")
    plt.ylabel("f1 score")
    plt.legend()
